...
func repack(t *testing.T, repo restic.Repository, packs restic.IDSet, blobs restic.BlobSet) {
	repackedBlobs, err := repository.Repack(context.TODO(), repo, repo, packs, blobs)
	if err != nil {
		t.Fatal(err)
	}
...
func rebuildIndex(t *testing.T, repo restic.Repository) {
	...
	_, err = repo.(*repository.Repository).CreateIndexFromPacks(context.TODO(), packs)
	if err != nil {
		t.Fatal(err)
	}
	...
	_, err = repo.Index().Save(context.TODO(), repo, restic.NewIDSet(), nil)
	if err != nil {
		t.Fatal(err)
	}
}
...
func testRepackCopy(t *testing.T, version uint) {
	repo := repository.TestRepositoryWithVersion(t)
	dstRepo := repository.TestRepositoryWithVersion(t)
	...
	_, err := repository.Repack(context.TODO(), repoWrapped, dstRepoWrapped, copyPacks, keepBlobs)
	if err != nil {
		t.Fatal(err)
	}
...
func testRepackWrongBlob(t *testing.T, version uint) {
	...
	_, err := repository.Repack(context.TODO(), repo, repo, rewritePacks, keepBlobs)
	if err == nil {
